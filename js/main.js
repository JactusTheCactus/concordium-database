...
function generateCharacterDetailPage(characterData, data) {
    ...
    infoElement.innerHTML = `Weapon: <u>${characterData.weapon}</u><br>Power: <u>${characterData.power}</u><br>Gear Colour: <u>${characterData.colour}</u><br>Species: <u>${characterData.species}</u><br>Inverse ${inverseCharacterData.alignment}: <u>${inverseCharacterData.name} <u>${inverseCharacterData.rank}</u><br>Epithet: <u>${characterData.epithet}</u>`
}
